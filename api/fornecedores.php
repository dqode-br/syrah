...
class Fornecedor {
    ...
    public function getNextCode() {
        ...
        return "FOR" . str_pad($next_number, 10, "0", STR_PAD_LEFT);
    }
}
...
switch($method) {
    case 'GET':
        if(isset($_GET['next_code'])) {
            echo json_encode(array("codigo" => $fornecedor->getNextCode()));
        } else {
            ...
            if($num > 0) {
                ...
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    array_push($fornecedores_arr, $row);
                }
